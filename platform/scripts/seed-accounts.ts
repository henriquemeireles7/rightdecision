  ...
  for (const account of accounts) {
    const existing = await db.query.platformAccounts.findFirst({
      where: and(
        ...
        eq(platformAccounts.accountHandle, account.accountHandle),
      ),
    })
    if (existing) {
      skipped++
