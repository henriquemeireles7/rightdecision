...
export async function getMetrics(uploadPostId: string): Promise<PostMetrics> {
  ...
  if (!apiKey) throw new ProviderError('upload-post-analytics', 'auth', 401, 'UPLOAD_POST_API_KEY not configured')
  ...
  try {
    ...
    if (response.status === 404 || response.status === 410) {
      throw new ProviderError('upload-post-analytics', 'getMetrics', response.status, 'Post not found or deleted')
    }
    if (response.status === 429) {
      throw new ProviderError('upload-post-analytics', 'getMetrics', 429, 'Rate limited')
    }
    if (response.status >= 500) {
      throw new ProviderError('upload-post-analytics', 'getMetrics', response.status, await response.text())
    }
    if (!response.ok) {
      throw new ProviderError('upload-post-analytics', 'getMetrics', response.status, await response.text())
    }
