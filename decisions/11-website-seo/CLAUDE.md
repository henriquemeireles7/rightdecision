# 11-website-seo

## Purpose
Strategy document: Website & SEO/GEO Strategy. Transforms rightdecisions.io from a landing page into a full company website with blog, technical SEO, GEO optimization, interactive tools, and mass content pipeline.

## Must-Read Context
Before working in this folder, read:
- decisions/roadmap.md — current priorities
- decisions/design.md — visual identity
- decisions/company.md — product context

## Additional Context
- decisions/voice.md — for any content/copy decisions
- decisions/02-manifesto/document.md — brand positioning, proprietary concepts
- decisions/lifedecisions/05-landing-page/document.md — existing LP spec

## Rules
- Follow the project-wide rules in the root CLAUDE.md.

---
<!-- AUTO-GENERATED BELOW — do not edit manually -->

## Files
| File | Exports |
|------|---------|
| action-plan.md | SEO/GEO Action Plan — What Henry Does Manually |
| document.md | Website & SEO/GEO Strategy — The Right Decision |
| input.md | INPUT: Website & SEO/GEO Strategy |
| meta.md | META-DOC: Website & SEO/GEO Strategy |
| raw.md | RAW FOUNDER ANSWERS — Website & SEO/GEO Strategy |

<!-- Generated: 2026-04-08T09:09:15.690Z -->
