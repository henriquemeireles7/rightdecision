...
describe('paymentConfirmationEmail', () => {
	test('returns branded receipt + first lesson CTA', () => {
		const email = paymentConfirmationEmail({ name: 'Maria', amount: '$197.00', renewalDate: 'April 7, 2027', firstLessonUrl: 'https://app.com/course/01' })
...
describe('renewalReceiptEmail', () => {
	test('returns branded renewal receipt', () => {
		const email = renewalReceiptEmail({ name: 'Maria', amount: '$197.00', cardLast4: '4242', nextRenewalDate: 'April 7, 2028', portalUrl: 'https://billing.stripe.com/p/xxx' })
		assertValidEmail(email)
...
describe('paymentFailedEmail', () => {
	test('returns helpful non-alarming email with portal link', () => {
		const email = paymentFailedEmail({ name: 'Maria', amount: '$197.00', portalUrl: 'https://billing.stripe.com/p/xxx' })
		assertValidEmail(email)
...
describe('renewalReminderEmail', () => {
	test('returns transparent reminder with card info and portal links', () => {
		const email = renewalReminderEmail({ name: 'Maria', amount: '$197.00', renewalDate: 'April 7, 2027', cardBrand: 'Visa', cardLast4: '4242', portalUrl: 'https://billing.stripe.com/p/xxx' })
		assertValidEmail(email)
...
describe('subscriptionCancelledEmail', () => {
	test('returns respectful cancellation email with resubscribe link', () => {
		const email = subscriptionCancelledEmail({ name: 'Maria', periodEndDate: 'April 7, 2027', pricingUrl: 'https://app.com/pricing' })
		assertValidEmail(email)
