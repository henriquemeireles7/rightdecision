...
const mockSelectFrom = mock(() => ({ where: mock(() => Promise.resolve([])), groupBy: mock(() => ({ having: mock(() => Promise.resolve([])) })) }))
...
describe('reminders', () => {
	...
	describe('sendModuleCompletionEmail', () => {
		it('sends email with module name', async () => {
			...
			expect(mockSendEmail).toHaveBeenCalledTimes(1)
			expect(mockSendEmail.mock.calls[0]![1]).toContain('The Wake-Up Call')
		})
		...
		it('falls back to generic module name for unknown module', async () => {
			...
			expect(mockSendEmail).toHaveBeenCalledTimes(1)
			expect(mockSendEmail.mock.calls[0]![1]).toContain('Module 99')
		})
	})
