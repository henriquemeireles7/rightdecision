...
export function getNavItems({ accessTier, isLoggedIn, isAdmin }: NavProps): NavItem[] {
  ...
  if (!isLoggedIn) {
    ...
    // Free / expired
    items.push({ label: 'Course', href: '/courses/life-decisions' })
    items.push({ label: 'Wins Board', href: '/wins' })
...
export function Navigation(props: NavProps) {
  ...
  return (
    <nav class="flex items-center justify-between px-6 py-4 border-b border-linen bg-surface-white">
      <a href="/" class="text-xl font-display text-ink">
        Right Decision
      </a>
      <div class="flex items-center gap-6">
        ...
        {props.userName && <span class="text-sm text-muted">{props.userName}</span>}
      </div>
    </nav>
  )
}
