...
const mockFindFirstClip = mock(() => Promise.resolve(null))
...
mock.module('@/platform/db/client', () => ({
  db: {
    query: {
      ...
      clips: { findFirst: () => mockFindFirstClip() },
    },
...
describe('features/(business)/post-distribute/service', () => {
  ...
  it('distributes posts successfully', async () => {
    ...
    mockFindFirstClip.mockResolvedValueOnce({ id: 'clip-1', storageUrl: 'https://r2.example.com/clips/clip-1.mp4' } as never)
