...
Include the line: "PR Quality Score: X/10" where X is the computed score.`,
      ...
      if (fs.existsSync(outputPath)) {
        ...
        const hasScore = content.toLowerCase().includes('quality score') || content.match(/\d+\/10/)
...
Write findings to ${dir}/review-output.md`,
      ...
      if (fs.existsSync(outputPath)) {
        ...
        const hasSqlFinding =
          content.includes('sql') || content.includes('injection') || content.includes('interpolat')
