...
function detectSeverity(category: Category): Severity {
	...
	if (category === 'deploy' && (lower.includes('fail') || lower.includes('crash'))) return 'critical'
...
if (lower.includes('lockfile') || lower.includes('frozen-lockfile')) {
	result.suggestions.push('Add lockfile-check to Stop hook: if package.json changed, verify bun.lock also changed')
}
if (lower.includes('dockerfile') && lower.includes('not found')) {
	result.suggestions.push('Add dockerfile-check script: validate runtime stage includes files needed by railway.toml commands')
}
if (lower.includes('process.env')) {
	result.suggestions.push('Existing hook should catch this — check if protect-files.ts covers this pattern')
}
if (lower.includes('merge conflict')) {
	result.suggestions.push('Add CLAUDE.md rule: rebase long-running branches before PR exceeds 10 files changed')
}
